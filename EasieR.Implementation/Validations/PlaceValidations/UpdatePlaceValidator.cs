...
{
    public class UpdatePlaceValidator : AbstractValidator<PlaceDto>
    {
        public UpdatePlaceValidator(EasieRContext context)
        {
            RuleFor(x => x.Name).NotEmpty().DependentRules(() =>
            {
                RuleFor(x => x.Name).Must((place, name) => !context.Place.Any(x => x.Name == name && x.Id != place.Id))
