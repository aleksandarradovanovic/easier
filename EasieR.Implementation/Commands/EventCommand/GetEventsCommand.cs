...
namespace EasieR.Implementation.Commands.EventCommand
{
    public class GetEventsCommand : IGetEventsQuery
    {
        ...
        public PagedResponse<EventDto> Execute(EventSearch search)
        {
            try
            {
                var eventItem = _easieRContext.Event.Include(x => x.ReservationTypes).Include(x=>x.EventImages).Include(x => x.Place).ThenInclude(x => x.Location).AsQueryable().Where(x => !x.isDeleted);
                if (search.Name != null)
                {
